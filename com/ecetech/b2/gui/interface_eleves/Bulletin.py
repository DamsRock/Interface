...
def entre(r, c, h, w, la):
    champ = tk.Label(root, text=str(la), height=h, width=w, relief=tk.RIDGE)
...
def xml_read():
    ...
    entre(row, mati, 1, cmw, 'Matières')
    entre(row, coef, 1, ccw, 'Coeff')
    entre(row, moye, 1, cmew, 'Moyenne élèves')
    ...
    for eleve in tree.findall('eleve'):
        ...
        root.title('Bulletin de '+prenom+' '+nom+'      Moyenne total = '+moyenne_total)
